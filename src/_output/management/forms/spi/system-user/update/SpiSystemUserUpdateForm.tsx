...
import { useSpiSystemUser } from "@/contexts/resources/spi/SpiSystemUserContext"
import { useToast } from "@/contexts/general/ToastContext"
import { SpiSystemUserUpdateRequest } from "@/types/api.types"
import useSpiSystemUserUpdateFields from "./useSpiSystemUserUpdateFields"
import SpiSystemUserUpdateLayout from "./SpiSystemUserUpdateLayout"

interface SpiSystemUserUpdateFormProps {
  systemUserId: number
  defaultValues?: Partial<SpiSystemUserUpdateRequest>
...
export default function SpiSystemUserUpdateForm(props: SpiSystemUserUpdateFormProps) {
  const {
    systemUserId,
    defaultValues,
    disabledFields,
  ...
  const { retrieve, update } = useSpiSystemUser()
  ...
  } = useForm<SpiSystemUserUpdateRequest>({
    mode: "onBlur",
  })

  const fields = useSpiSystemUserUpdateFields({ errors, disabledFields, selectFilterBys, selectOrderBys })
  ...
  const onSubmit: SubmitHandler<SpiSystemUserUpdateRequest> = async (data) => {
    setLoading(true)
    setApiErrors([])
    try {
      await update(data)
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <FormTemplate
        control={control}
        fields={fields}
        layout={SpiSystemUserUpdateLayout}
